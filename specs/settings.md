...
`Base directory` — обязательный абсолютный путь на удалённом сервере, внутри которого создаются папки задач. Настройка сохраняется как `tasks.base_dir` и доступна для изменения. Init и агент всегда выполняются с `cwd` `<tasks.base_dir>/<TASK-ID>` по [agent.md](agent.md); местоположение файла скрипта не меняет cwd.
