...
- При запуске через `start.py` прокси слушают тот же bind-адрес, что и web-приложение, включая `0.0.0.0`/`::`: адрес передаётся через APP_WEB_HOST/APP_WEB_PORT. Прямой CLI Uvicorn читается из аргументов. При программном запуске без этих переменных используется `127.0.0.1`, а порт основного приложения считается неизвестным; произвольный программный bind автоматически не определяется.
...
Структурный прототип блока Proxy и элементов управления:
...
- Положительное целое число секунд, default `30`. В UI поле обязательно, пустая строка в API недопустима; если ключ timeout отсутствует в API/сохранённом блоке, применяется `30`.
...
### Уточнения текущего UI и границ применения

В отличие от схематичного PNG используются общие скруглённые поля, светлый фон и фиолетовая Save. `Add proxy` и `Remove proxy` выровнены по началу области полей, Save — справа. Обычные ошибки Save показываются в Toast с номером блока, без отдельной ошибки рядом с полем; ошибки старта listeners выводятся списком над формой. На мобильной ширине название Proxy переносится над полями, port и timeout остаются в одной строке.

Блокировка Save HTTP proxy — `threading.Lock` экземпляра `ProxyServers`, то есть внутри одного web-процесса. Файл заменяется атомарно; межпроцессная `settings.locking.configuration_lock` к этой секции не применяется. Горячее обновление действует только на listeners того экземпляра, который обработал Save. Штатный запуск использует один web-worker.

Upstream-статус 101 преобразуется в 502; WebSocket не обслуживается. Проверка допустимых HTTP-методов работает после разбора запроса сервером Uvicorn: синтаксически некорректный HTTP может быть отклонён раньше обработчика proxy.
