...
Новые задачи добавляются в конец BACKLOG; Save со сменой статуса — в конец целевой колонки. Перетаскивание задаёт позицию и пересчитывает порядок. Кнопки REOPEN/DONE/To Archive передают `position: 0`. Send comment меняет статус на OPEN, сохраняя прежний `sort_order`.
