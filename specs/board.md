...
- При временной блокировке SQLite чтение снимка делает до трёх программных попыток с новым соединением и транзакцией, сохраняя встроенное ожидание до 10 секунд на заблокированную операцию и паузу 100 мс между попытками. При успехе сразу возвращает снимок; после третьей ошибки блокировки передаёт последнее исходное исключение SQLite. Другие ошибки не повторяются. Требования к ожиданию — [tech-spec.md](tech-spec.md#ожидание-блокировки-sqlite).
...
Переход `OPEN → BACKLOG` доступен перетаскиванием карточки и через Status → BACKLOG → Save в модальной форме и на отдельной странице задачи. После перехода задача исключается из очереди захвата агента до явного возврата в OPEN. Save сохраняет поля и вложения по общим правилам; перетаскивание меняет только статус и порядок.

Проверки: `tests/test_board.py` покрывает PATCH, JSON/multipart Save, порядок карточек, сохранение вложений и исключение BACKLOG из захвата. `tests/board_live.js` проверяет переход через обе формы и drag&drop на реальном API/SSE, сохранение после перезагрузки и отсутствие горизонтального переполнения отдельной формы на 375px.
