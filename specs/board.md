...
- При захвате задачи исполнителем карточка автоматически перемещается из `OPEN` в `IN PROGRESS`; обновляются счётчики, цвет и индикатор `In progress`. Завершение обновляет её до `REVIEW`, ошибка — до `WAIT` с признаком ошибки. Создание, изменение, архивирование и удаление в других клиентах также отражаются на Board.
