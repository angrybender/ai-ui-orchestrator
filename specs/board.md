...
Прототип дизайна блока задачи, который помещается на доску: ![board-task-block.png](board-task-block.png)
...
- %TASK-TITLE% - название задачи, в блоке на доске режется до двух строк с учетом пробелов (при truncate добавить "..." в конце)
...
Прототип дизайна формы создания/редактирования задачи: ![task-edit-create-form.png](task-edit-create-form.png)

> File upload : возможно загрузки файла 
...
- Переход в `ARCHIVE` допустим только из `BACKLOG` и `DONE`.
...
*%TASK_ID%*
- уникальный, по умолчанию сгенерировать так: взять последнюю по списку задачу из базы и прибавить к числовой части 1 
- при конфликте идентификаторов прибавлять 1 до тех пор, пока идентификатор станет уникальным
...
- параметр конфигурации, задающий префикс: `board.task_prefix`
...
- Переход в `ARCHIVE` разрешён только из `BACKLOG` и `DONE`; переходы из `ARCHIVE` в другие статусы через UI запрещены.

*Дата создания*
- В формате дата-время.
- Выводить в формате ГОД-МЕСЯЦ-ДЕНЬ ЧАС:МИНУТА (пример "2026-08-12 14:31")
...
Красная карточка ошибки, включая Init, находится в `WAIT`; отдельного статуса ERROR нет. В эталоне `board-task-block.png` исправлена подпись колонки красной карточки на WAIT. Ручной переход IN PROGRESS → BACKLOG отменяет Init; поздний результат не возвращает карточку в WAIT/REVIEW и не меняет новый запуск. Повтор после ошибки — через Send comment из WAIT. Правила выполнения и отмены — [agent.md](agent.md).
